...
1. Step 1: Generate a heuristic (translation) file skeleton and some associated descriptor text files.
2. Step 2: Modify the *heuristic.py* to specify BIDS output names and directories, and the input DICOM characteristics.
3. Step 3: Call HeuDiConv to run on more subjects and sessions.
...
```{bash}
curl -L https://nx81903.your-storageshare.de/s/fKw8zSoLwGoDmyE/download -o rdm-data.zip
unzip -j -o rdm-data.zip -d rdm-data
rm -f rdm-data.zip
cd rdm-data
```
...
Step 1 only needs to be completed once for each project.
...
* `-f convertall` This creates a *heuristic.py* template from an existing heuristic module.
There are [other heuristic modules](https://github.com/nipy/heudiconv/tree/master/heudiconv/heuristics), but *convertall* is a good default.
...
It is okay to rename this file, or to have several versions with different names, just be sure to pass the intended filename with `-f`.
...
* **Template**
  ...
  * Use the variable `{session}` to make the code general purpose only if you have multiple sessions for each subject.
    ...
    * Otherwise you will get [bids validator errors](https://bids-standard.github.io/bids-validator/)

  * Define the output directories and file names according to the [BIDS specification](https://bids-specification.readthedocs.io/en/stable/modality-specific-files/magnetic-resonance-imaging-data.html)
  ...
  * Data that is not yet defined in the BIDS specification will cause the bids-validator to produce an error unless you include it in a [.bidsignore](https://github.com/bids-standard/bids-validator?#bidsignore) file.
...
```{bash}
heudiconv -d "dicom/{subject}/*/*.dcm" -o Nifti/ -f Nifti/code/heuristic2.py -s 219 -ss itbs -c dcm2niix -b --minmeta --overwrite
```
...
* This command is slightly different from the previous command you ran.
...
::: {.callout-note}
*image_type* is described in the [DICOM specification](https://dicom.innolitics.com/ciods/mr-image/general-image/00080008)
:::